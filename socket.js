...
module.exports = {
  init: (server) => {
    ...
    // ============================================================
    // `ensureCheckedInToday`'s staleness check (attendanceCleanup.js)
    // handles the common case: it runs every time a client reconnects
    // or heartbeats, and closes out an abandoned session the moment
    // someone comes back. This sweep covers the case where NOBODY
    // ever comes back for a while (person goes on leave with a dead
    // laptop, etc.) — without it, a session could sit open in the DB
    // indefinitely with nothing to trigger the staleness check.
    // Runs every 15 minutes; closes any session whose `loginTime` is
    // older than 16h with no clean close.
    // ============================================================
    setInterval(async () => {
      try {
        const { closeStaleOpenSessions } = require("./controllers/attendanceCleanup");
        const result = await closeStaleOpenSessions();
        if (result.sessionsClosed > 0) {
          console.log(
            `Stale session sweep: closed ${result.sessionsClosed} session(s) across ${result.recordsTouched} record(s).`,
          );
        }
      } catch (err) {
        console.error("Stale session sweep failed:", err.message);
      }
    }, 15 * 60 * 1000);
