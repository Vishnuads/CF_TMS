...
// Lightweight — just who currently has a running task timer, keyed by
// employee id. Lets the dashboard show live badges without pulling each
// employee's full analytics payload.
router.get("/live-status", async (req, res) => {
  try {
    const activeTasks = await Task.find({ timerRunning: true })
      .select("title assigned_to timerStartedAt totalTimeSpent project status")
      .populate("project", "name")
      .lean();
    ...
    activeTasks.forEach((task) => {
      (task.assigned_to || []).forEach((userId) => {
        ...
        // If an employee somehow has more than one running timer, surface
        // whichever task they've been on the longest.
        if (!map[id] || new Date(task.timerStartedAt) < new Date(map[id].timerStartedAt)) {
          map[id] = {
            taskId: task._id,
            title: task.title,
