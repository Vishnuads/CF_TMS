...
// ============================================================
// ENSURE CHECKED-IN TODAY
// ============================================================
//
// Called from socket.js on `join-user` (fresh connect / reconnect)
// and `presence-ping` (heartbeat while a tab is alive). Its job is to
// reopen today's attendance session if it was closed while this tab
// was unreachable (short sleep, brief network drop, the disconnect
// timer or daily sweep force-closing it) — WITHOUT ever creating a
// duplicate session while one is already open.
//
// recordCheckIn() is already fully idempotent (see above: it only
// pushes a new session when none is currently open), so this can
// simply delegate to it directly. Keeping it as a separate named
// export (rather than having socket.js call recordCheckIn directly)
// keeps the two call sites' *intent* distinct in the code even though
// the underlying behavior is identical — "a user explicitly checked
// in" vs. "we're just making sure today's session is still open".
// ============================================================

async function ensureCheckedInToday(userId, userName) {
  return recordCheckIn(userId, userName);
}
