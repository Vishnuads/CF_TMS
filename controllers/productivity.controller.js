...
exports.getEmployeeProductivity = async (req, res) => {
  try {
    ...
    // so the attendance table and the Schedule Adherence score stay
    ...
    const attendance = result.raw.attendance.map((record) => {
      ...
      // IMPORTANT:
      // If the latest session is active, don't show an old logout time.
      const latestSessionIsActive = latestSession && !latestSession.logoutTime;

      const closedSessions = sessions.filter((session) => session.logoutTime);

      const lastCheckOut = latestSessionIsActive
        ? null
        : closedSessions.length > 0
          ? closedSessions.reduce((latest, current) =>
              new Date(current.logoutTime).getTime() >
      ...
      return {
        ...
        onTimeArrival: firstCheckIn ? isOnTimeLogin(firstCheckIn) : false,
    ...
    const tasks = result.raw.tasks.map((task) => {
      ...
      return {
        ...
        assignedTime: Number(task.assignedTime || 0),
        completedAt: task.completedAt || null,
        onTime,
        totalTimeSpent: Number(task.totalTimeSpent || 0),
        totalTimeSpentLabel: fmtDuration(task.totalTimeSpent || 0),

        timerRunning: Boolean(task.timerRunning), // NEW
        timerStartedAt: task.timerStartedAt || null, // NEW
      };
    });

    // ========================================================
    // ACTIVE TASKS ("worked on in this period")
    //
    // Independent of assignedDate/dueDate — driven entirely by
    // timeSessions / a currently-running timer overlapping the
    // selected window. See productivityScore.service.js for the
    // computeTaskActiveTime() logic.
    // ========================================================

    const activeTasks = (result.raw.activeTasks || []).map((task) => ({
      _id: task._id,
      title: task.title,
      ...
      activeSecondsLabel: fmtDuration(task.activeSeconds),
    }));
