...
//           return new Date(current.logoutTime) >
...
//   // IMPORTANT:
//   // Calculate actual session durations, not first login → last logout
...
//     isCurrentlyActive: sessions.some(
//       (session) => !session.logoutTime
//     ),
//   };
// });
...
function resolvePeriod(query) {
  ...
  const granularity =
    VALID_GRANULARITIES.includes(query.period)
      ? query.period
      : "monthly";
  ...
  if (granularity === "weekly") {
    ...
    if (
      ...
      if (!startDate || !endDate) {
        return {
          error:
            "Invalid startDate/endDate. Expected format: YYYY-MM-DD",
        };
      }
...
exports.getEmployeeProductivity =
  async (req, res) => {
    try {
      const { userId } =
        req.params;
