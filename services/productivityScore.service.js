...
// ============================================================
// Rule:
//   loginTime <  10:00:00 AM  →  ON TIME
//   loginTime >= 10:00:00 AM  →  LATE
// ============================================================

const SCHEDULE_CUTOFF_HOUR = 10;
const SCHEDULE_CUTOFF_MINUTE = 0;

/**
 * Builds a Date object for exactly 10:00:00.000 AM on the SAME
 * calendar day as `referenceDate`. This guarantees the cutoff is
 * always evaluated against that specific day's 10 AM — not just
 * an hour-number comparison — so there's zero ambiguity at the
 * boundary itself.
 */
function getCutoffTimeFor(referenceDate) {
  const cutoff = new Date(referenceDate);
  cutoff.setHours(SCHEDULE_CUTOFF_HOUR, SCHEDULE_CUTOFF_MINUTE, 0, 0);
  return cutoff;
}

/**
 * Returns true if loginTime is strictly BEFORE 10:00:00 AM
 * (on time), false if it is AT or AFTER 10:00:00 AM (late).
 */
function isOnTimeLogin(loginTime) {
  if (!loginTime) return false;
  const login = new Date(loginTime);
  const cutoff = getCutoffTimeFor(login);
  return login.getTime() < cutoff.getTime();
}
